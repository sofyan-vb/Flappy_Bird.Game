...
export class GameEngine {
  ...
  startWithDifficulty(speed: number, interval: number) {
    this.soundManager.init(); this.gameSpeed = speed; this.pipeSpawnInterval = interval;
    const ratio = speed / 250.0; this.bird.gravity = 1500 * ratio; this.bird.jumpVelocity = -500 * ratio;
    this.startGame();
  }
  ...
  goToMainMenu() { this.setState(GameState.mainMenu); this.bird.reset(); this.pipeManager.reset(); }
  gameOver() { if (this.gameState === GameState.gameOver) return; this.soundManager.playHit(); this.setState(GameState.gameOver); }
  ...
  onTap() {
    ...
    if (this.gameState === GameState.playing) { this.bird.jump(); this.soundManager.playFlap(); }
    else if (this.gameState === GameState.mainMenu) { this.startGame(); }
  }
  ...
  render() {
    ...
    if (this.currentTheme === GameTheme.classic) {
      ...
      sunGlow.addColorStop(0.3, 'rgba(255, 235, 59, 0.8)');
      sunGlow.addColorStop(1, 'rgba(255, 235, 59, 0)');
      this.ctx.fillStyle = sunGlow;
      this.ctx.beginPath(); this.ctx.arc(sunX, sunY, 60, 0, Math.PI * 2); this.ctx.fill();
      

      this.ctx.fillStyle = '#FFF59D';
      this.ctx.beginPath(); this.ctx.arc(sunX, sunY, 20, 0, Math.PI * 2); this.ctx.fill();
      ...
      this.ctx.fillRect(0, waterY, this.width, 3);
      ...
      const dSunX = this.width * 0.75;
      const dSunY = this.height * 0.3;
      const dSunGlow = this.ctx.createRadialGradient(dSunX, dSunY, 20, dSunX, dSunY, 100);
      dSunGlow.addColorStop(0, 'rgba(255, 255, 255, 1)');
      dSunGlow.addColorStop(0.2, 'rgba(255, 245, 157, 0.8)');
      dSunGlow.addColorStop(1, 'rgba(255, 138, 101, 0)');
      this.ctx.fillStyle = dSunGlow;
      this.ctx.beginPath(); this.ctx.arc(dSunX, dSunY, 100, 0, Math.PI * 2); this.ctx.fill();
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(this.width * 0.1, duneY);
      this.ctx.lineTo(this.width * 0.35, duneY - 110);
      this.ctx.lineTo(this.width * 0.6, duneY);
      this.ctx.fill();
      
      this.ctx.fillStyle = '#EF6C00'; 
      this.ctx.beginPath();
      this.ctx.moveTo(this.width * 0.45, duneY);
      this.ctx.lineTo(this.width * 0.6, duneY - 80);
      this.ctx.lineTo(this.width * 0.8, duneY);
      this.ctx.fill();
      ...
      this.ctx.fillStyle = backDune;
      this.ctx.beginPath();
      this.ctx.moveTo(0, duneY);
      this.ctx.quadraticCurveTo(this.width * 0.3, duneY - 100, this.width * 0.7, duneY - 30);
      this.ctx.quadraticCurveTo(this.width * 0.9, duneY - 60, this.width, duneY - 80);
      this.ctx.lineTo(this.width, duneY);
      this.ctx.fill();
      ...
      this.ctx.fillStyle = frontDune;
      this.ctx.beginPath();
      this.ctx.moveTo(0, duneY - 20);
      this.ctx.quadraticCurveTo(this.width * 0.2, duneY + 20, this.width * 0.6, duneY - 70);
      this.ctx.quadraticCurveTo(this.width * 0.8, duneY - 90, this.width, duneY - 40);
      this.ctx.lineTo(this.width, duneY);
      this.ctx.lineTo(0, duneY);
      this.ctx.fill();
